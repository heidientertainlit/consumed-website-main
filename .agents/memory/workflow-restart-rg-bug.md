...
**Why:** Platform-side stale state — the referenced skill dirs don't exist anywhere in the workspace; creating placeholder dirs does NOT fix it (tried both in `workspace/.local/skills` and in the `/mnt/*/working_subv/.local/skills` mount — the failing rg runs in a scope not reachable from the shell). `configureWorkflow` fails with the same error. Retrying just reproduces it.
